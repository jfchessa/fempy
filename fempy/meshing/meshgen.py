    ...
    for t in eta:
        for s in xi:
            N = 0.25*np.array([(1-s)*(1-t),(1+s)*(1-t), \
                    (1+s)*(1+t),(1-s)*(1+t)], dtype=fp.FLOAT_TYPE )
        ...
        for s in eta:
            for r in xi:
                N =  eigth*np.array( [(1-r)*(1-s)*(1-t), \
                     ...
                     (1+r)*(1-s)*(1+t), \
                     (1+r)*(1+s)*(1+t), \
                     (1-r)*(1+s)*(1+t) ], dtype=fp.FLOAT_TYPE )
                pts[n,:]=np.dot(N,corners)
                n = n + 1
...
def gen_conn2d(ptrn,nu,nv,incu=1,incv=2):
    ...
    nc = ptrn.size
    conn = np.zeros( (nu*nv,nc), dtype=fp.INDX_TYPE )
    inc = np.zeros( nc, dtype=fp.INDX_TYPE )
    e=0
    for r in xrange(nv):
        for c in xrange(nu):
            conn[e] = ptrn + inc
...
def gen_conn3d(ptrn,nu,nv,nw,incu=1,incv=2,incw=None):
    ...
    nc = ptrn.size
    conn = np.zeros( (nu*nv*nw,nc), dtype=fp.INDX_TYPE )
    inc = np.zeros( nc, dtype=fp.INDX_TYPE )
    e=0
    for l in xrange(nw):
        for r in xrange(nv):
...
class MeshQuad4(object):
    ...
    def __init__(self,corners,n1,n2,prop=0,bias1=BiasNone(),bias2=BiasNone()):
        ...
        # define elements
        ptrn = np.array( [0,1,n1+1,n1], dtype=fp.INDX_TYPE )
    ...
    def EdgeNIDs(self,edgeid):
        ...
        """
        if ( edgeid == 0 ):
            edgeNodes = np.array( range(0,self.n1), fp.INDX_TYPE )
        
        elif ( edgeid == 1 ):  
            edgeNodes = np.array( range(self.VertexNID(1),self.NumNodes(),self.n1), fp.INDX_TYPE )
            
        elif ( edgeid == 2 ):    
            edgeNodes = np.array( range(self.VertexNID(2),self.VertexNID(3)-1,-1), fp.INDX_TYPE )
            
        elif ( edgeid == 3 ):   
            edgeNodes = np.array( range(self.VertexNID(3),-1,-self.n1), fp.INDX_TYPE )
            
        else:
            print "Edge id must be between 0 and 3"
            return np.array( [], fp.INDX_TYPE )
    ...
    def EdgeElem(self,edgeid):
        ...
        for n1 in edgeNodes[1:]:
            edgeElem.append( fp.ElemLine2( np.array([n0,n1], fp.INDX_TYPE ), prop ))
            n0 = n1
        return edgeElem      
    ...
    def EdgeNormals(self,edgeid):
        ...
        normals = []
        ve=np.zeros(3,fp.FLOAT_TYPE)
        vt=np.zeros(3,fp.FLOAT_TYPE)
        v=np.zeros(3,fp.FLOAT_TYPE)
...
class MeshHexa8(MeshQuad4):
    ...
    def __init__(self,corners,n1,n2,n3,prop=0,bias1=BiasNone(),bias2=BiasNone(),bias3=BiasNone()):
        ...
        # define nodes
        ncoord = node_array3d(corners,n1,n2,n3,bias1,bias2,bias3)
        self.node = fp.NodeArray( narray=ncoord )
        
        dn=n1*n2
        ptrn = np.array( [0,1,n1+1,n1, dn,1+dn,n1+1+dn,n1+dn], dtype=fp.INDX_TYPE )
        conn = gen_conn3d(ptrn,n1-1,n2-1,n3-1)
        self.element = fp.ElementArray(conn=conn,etype=fp.ElemHexa8,prop=prop)
        
     #   self.element = []
     #   for e in xrange( self.conn.shape[0] ):
     #       self.element.append( fp.ElemHexa8( self.conn[e], self.prop ) ) 
     #
     #   # define elements
     #   ptrn = np.array( [0,1,n1+1,n1], dtype=fp.INDX_TYPE )
     #   conn = gen_conn2d(ptrn,n1-1,n2-1)
    ...
    def FaceElem(self,faceid):
        ...
        if ( faceid==0 ):
            n0 = self.VertexNID(1)
            dz=self.n1*self.n2
            ptrn = np.array( [n0,n0+self.n1,n0+dz+self.n1,n0+dz], dtype=fp.INDX_TYPE )
            ...
            n0 = self.VertexNID(3)
            dz=self.n1*self.n3
            ptrn = np.array( [n0,n0+1,n0+dz+1,n0+dz], dtype=fp.INDX_TYPE )
            ...
            n0 = self.VertexNID(4)
            ptrn = np.array( [n0,n0+1,n0+self.n1+1,n0+self.n1], dtype=fp.INDX_TYPE )
        ...
        felem = []
        for ec in fconn:
            felem.append( fp.ElemQuad4( ec, self.prop ) )
